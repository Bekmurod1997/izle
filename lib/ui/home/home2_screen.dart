...
class _HomeScreen2State extends State<HomeScreen2> {
  ...
  final AllAdvertisementsController advertisementsController =
      Get.find<AllAdvertisementsController>();
  ...
  @override
  void initState() {
    advertisementsController.fetchAllAds();
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels ==
        ...
        if (!advertisementsController.isLoadMore.value) {
          advertisementsController.loadMore(page + 1);
          setState(() {
            page += 1;
          });
        }
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      if (advertisementsController.isLoading.value) {
        return Center(
            ...
            children: [
              ...
              Padding(
                ...
                child: GridView.builder(
                  ...
                  shrinkWrap: true,
                  itemCount: advertisementsController.allAdsList.length,
                  gridDelegate:
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () => Get.to(
                        () => ProductDetailScreen(proId: 1),
                        // proId: adsController.allAdsList().data![index].id)
                      ),
                      child: RecommandationItem(
                        isFavorite: false,
                        title: advertisementsController.allAdsList.first.title
                            .toString(),
                        // adver.allAdsList().data?[index].title ?? '',
                        id: advertisementsController.allAdsList.first.id!,
                        city: advertisementsController
                                .allAdsList.first.cityName ??
                            'tashkent',
                        price: advertisementsController.allAdsList.first.price
                            .toString(),
                        date: advertisementsController.allAdsList.first.date!,
                        imageUrl:
                            advertisementsController.allAdsList.first.photo!,
                      ),
                    );
                  },
                ),
